...
<div class="hero">
  ...
  <div class="badges">
    <span class="badge rounded-pill">C++20</span><span class="badge rounded-pill">Linux · macOS · Windows · Android · iOS · WebAssembly</span><span class="badge rounded-pill">MIT</span><span class="badge rounded-pill">HTTP/S</span><span class="badge rounded-pill">zero required deps</span>
  </div>
...
C++ has no shortage of HTTP libraries, but shipping one usually means dragging in libcurl, OpenSSL, and a chain of transitive dependencies. mog takes the opposite approach. On native platforms it prefers the HTTP stack the operating system already ships: libcurl on Linux (loaded at runtime), NSURLSession on macOS and iOS, and WinHTTP on Windows. When none of those is available, it falls back to a small **embedded** stack (HTTP/1.1 + mbedTLS). Browser WebAssembly builds use Fetch and the browser's TLS implementation.

The result is a client you can **link into a desktop, Android NDK, iOS, or WebAssembly application**. A native binary can run in a `FROM scratch` container with nothing else on disk; HTTPS still verifies there because the Mozilla CA bundle is compiled in. Android releases are Prefab AARs, iOS releases are XCFrameworks, and browser HTTPS uses browser trust subject to CORS and mixed-content policy.
...
- **iOS.** A static XCFramework exposes the same C++ client and server APIs to arm64 devices and arm64/x86_64 Simulator.
...
<div class="row row-cols-1 row-cols-md-2 g-4 feature-grid">
  ...
  <div class="col"><div class="card h-100"><div class="card-body"><h3 class="card-title">iOS</h3><p class="card-text">XCFramework for devices and Simulator, with NSURLSession, embedded fallback, and Simulator-tested packaging.</p></div></div></div>
...
See the **[Guide](guide.html)** for backend selection and native deployment, the **[Android guide](android.html)** for Prefab integration, the **[iOS guide](ios.html)** for XCFramework integration, the **[Web / Emscripten guide](web.html)** for browser behavior, and the **[CLI Manual](cli.html)** for every flag.
